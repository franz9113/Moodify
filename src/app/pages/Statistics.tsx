...
import { format, subMonths, startOfMonth, endOfMonth, eachDayOfInterval, startOfWeek, endOfWeek, subWeeks, startOfYear, endOfYear, eachMonthOfInterval, subYears } from "date-fns";
  ...
  const weeklyTrend = useMemo(() => {
    ...
    return days.map((day, index) => {
      ...
      if (dayEntries.length > 0) {
        avgMood = dayEntries.reduce((sum, entry) => sum + getMoodValue(entry.mood), 0) / dayEntries.length;
      }
  ...
  const yearlyTrend = useMemo(() => {
    ...
    return months.map((month, index) => {
      ...
      if (monthEntries.length > 0) {
        avgMood = monthEntries.reduce((sum, entry) => sum + getMoodValue(entry.mood), 0) / monthEntries.length;
      }
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`px-4 py-2 rounded-lg transition-all ${
                viewMode === "week" ? "bg-cyan-500 text-white" : "bg-gray-100 text-gray-700"
              }`}
            >
              Week
            </button>
            <button
              ...
              className={`px-4 py-2 rounded-lg transition-all ${
                viewMode === "month" ? "bg-cyan-500 text-white" : "bg-gray-100 text-gray-700"
              }`}
            >
              Month
            </button>
            <button
              ...
              className={`px-4 py-2 rounded-lg transition-all ${
                viewMode === "year" ? "bg-cyan-500 text-white" : "bg-gray-100 text-gray-700"
              }`}
            >
              Year
            </button>
        ...
        <div className="px-6 py-6 space-y-6">
          ...
          <div className="bg-gradient-to-br from-cyan-50 to-teal-50 rounded-2xl p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              {mostFrequentMood && mostFrequentMood.count > 0 && (
                <div>
                  <img src={mostFrequentMood.image} alt={mostFrequentMood.mood} className="w-12 h-12" />
                  <p className="text-sm text-gray-600">Most Frequent</p>
                  <p className="text-xs text-gray-500">{mostFrequentMood.mood}</p>
                </div>
              )}
            </div>
          </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => setCurrentWeek(subWeeks(currentWeek, 1))} className="p-1">
                    <ChevronLeft size={16} />
                  </button>
              ...
              <div className="bg-white rounded-xl p-4 border border-gray-200">
                <ResponsiveContainer width="100%" height={200}>
                  <LineChart data={weeklyTrend} key={`week-${format(weekStart, 'yyyy-MM-dd')}`}>
                    ...
                    <Tooltip
                      content={({ active, payload }) => {
                        if (active && payload && payload.length) {
                          ...
                          const moodLabel = value >= 4.5 ? "Very Positive" : value >= 3.5 ? "Positive" : value >= 2.5 ? "Neutral" : value >= 1.5 ? "Low" : value > 0 ? "Very Low" : "No entries";
                          return (
                            <div className="bg-white p-3 rounded-lg shadow-lg border border-gray-200">
                              <p className="font-medium">{payload[0].payload.day}</p>
                              <p className="text-sm text-gray-600">{moodLabel}</p>
                              {count > 0 && <p className="text-xs text-gray-500">{count} {count === 1 ? 'entry' : 'entries'}</p>}
                            </div>
                          );
                        }
                        return null;
                    ...
                    />
                    <Line type="monotone" dataKey="mood" stroke="#06b6d4" strokeWidth={2} dot={{ fill: "#06b6d4", r: 4 }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-gray-600">{format(currentMonth, "MMMM yyyy")}</span>
                  <button
                    onClick={() => {
              ...
              <div className="bg-white rounded-xl p-4 border border-gray-200">
                <ResponsiveContainer width="100%" height={200}>
                  <LineChart data={monthlyTrend} key={`month-${format(monthStart, 'yyyy-MM')}`}>
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="day" />
                    <YAxis domain={[0, 5]} ticks={[0, 1, 2, 3, 4, 5]} />
                    <Tooltip
          ...
          {viewMode === "year" && (
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => setCurrentYear(subYears(currentYear, 1))} className="p-1">
                    <ChevronLeft size={16} />
                  </button>
                  ...
                  <button
                    ...
                    className="p-1"
                    disabled={startOfYear(subYears(currentYear, -1)) > startOfYear(new Date())}
                  >
                    <ChevronRight size={16} />
                  </button>
                </div>
              ...
              <div className="bg-white rounded-xl p-4 border border-gray-200">
                <ResponsiveContainer width="100%" height={200}>
                  <LineChart data={yearlyTrend} key={`year-${format(yearStart, 'yyyy')}`}>
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="month" />
                    <YAxis domain={[0, 5]} ticks={[0, 1, 2, 3, 4, 5]} />
                    <Tooltip
                    ...
                    <Tooltip 
                      content={({ active, payload }) => {
                        if (active && payload && payload.length) {
                          return (
                            <div className="bg-white p-3 rounded-lg shadow-lg border border-gray-200">
                              <p className="font-medium">{payload[0].payload.mood}</p>
                              <p className="text-sm text-gray-600">{payload[0].value} times</p>
                            </div>
                          );
                        }
                        return null;
          ...
          {allEntries.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-sm text-gray-400 mt-2">Start tracking your moods to see statistics</p>
            </div>
          )}
        </div>
      </div>
