...
const suggestions: Record<string, { title: string; description: string; tool: string }> = {
  Happy: {
...
export default function Suggestions() {
  ...
  useEffect(() => {
    ...
    const completedEntry: MoodEntry = {
      ...
      date: localStorage.getItem("currentMoodDate") || new Date().toISOString().split("T")[0],
      mood: data.mood,
      ...
      <div className="flex-1 overflow-y-auto px-6 py-8">
        <div className="max-w-md mx-auto">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-lg mb-6">
            ...
            <h3 className="text-lg font-medium mb-2">{moodData.suggestion.title}</h3>
            <p className="text-gray-600 mb-4">{moodData.suggestion.description}</p>

            <div className="bg-cyan-50 rounded-lg p-4">
              <p className="text-sm text-gray-600 mb-1">Recommended tool:</p>
              <p className="font-medium text-cyan-700">{moodData.suggestion.tool}</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-lg">
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="font-medium">{moodData.mood} - {moodData.emotion}</p>
              </div>
              {moodData.whatMadeYouFeel && (
                <div>
                  <p className="text-sm text-gray-600">What made you feel this way</p>
